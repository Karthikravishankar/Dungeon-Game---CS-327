...
int main(int argc, char *argv[])
{
 ...
 if (argc > 1) {
  ...
  d.PC->potspeed=0;
  d.PC->potdamage=0;
  //set pc hp above this!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  gen_objects(&d);


  //object live = new object(objectdescription, pairt, d->objmap[p[dim_y]][p[dim_x]]);
  ...
  while (pc_is_alive(&d)  && !d.quit && d.deadboss ==0) {
  //while (pc_is_alive(&d) && dungeon_has_npcs(&d) && !d.quit && d.deadboss ==0) {
    do_moves(&d);
  }
  io_display(&d);
  ...
  if(d.deadboss ==1)
    {
      printf("You killed the game's Boss!\n");
    }
